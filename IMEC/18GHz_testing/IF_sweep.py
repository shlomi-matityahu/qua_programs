"""IF sweep program for the 18GHz testing
Two channels to to a 3-port mixer. 
"resonator1" is the "LO" port of the mixer.
"resonator2" is the "IF" port of the mixer.
The IF frequency or "resonator2" is swept from -500 MHz to +500 MHz in 100 MHz steps.
...
IF_values: list[int] = [i * step_hz for i in range(-3, 4)]  # -500 … +500 MHz in 100 MHz steps

with program() as IF_sweep:
    freq = declare(int)

    play("cw", "resonator1_sticky") # the "LO" of the mixer
    with for_each_(freq, IF_values):
        update_frequency("resonator2_sticky", freq) 
        play("cw", "resonator2_sticky") # the "IF" of the mixer
        pause()
...
if simulate:
    ...
    job = qmm.simulate(config, IF_sweep, simulation_config)
else:
    job = qm.execute(IF_sweep)
    print(job.get_job_id())

    # One pause() per IF step; host must call resume() to reach the next frequency
    for i, freq in enumerate(IF_values):
        while not job.is_paused():
            time.sleep(0.1)
        # print(f"{freq}")
        print(f"Paused at F = {(LO2 + freq) / 1e9:.2f} GHz ({i + 1}/{len(IF_values)})")
