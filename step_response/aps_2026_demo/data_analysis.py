...
def sequential_exp_fit(
    ...
    for i, start_frac in enumerate(start_fractions):
        # Calculate start index for this component (clip to avoid IndexError when fraction is 1.0)
        start_idx = np.clip(int(len(t) * start_frac), 0, len(t) - 1)
        if verbose:
...
square_pulse_3m_vs_3m_with_biastee = np.loadtxt("Square_pulse_3m_vs_3m_with_biastee.csv", delimiter=",")
# %%
plt.figure(figsize=(10, 6))
t_data = square_pulse_3m_vs_3m_with_biastee[:, 0] * 1e9 # convert to ns
# square_pulse_3m_vs_12m[:, 0] += 2
# square_pulse_3m_vs_12m[:, 2] += 0.06
mask_t1 = (t_data > 0) & (t_data < 7.5e3)
mask_v1 = square_pulse_3m_vs_3m_with_biastee[:, 1] > 0

# To use all time points, simply create a mask that selects all indices.
# For example, for an array `t`:
mask = np.ones_like(square_pulse_3m_vs_3m_with_biastee[:, 0], dtype=bool)
# mask = mask_t1 & mask_v1  # only positive x and y

t_data = t_data[mask]
y_data_3m = square_pulse_3m_vs_3m_with_biastee[mask, 1]
y_data_3m_with_biastee = square_pulse_3m_vs_3m_with_biastee[mask, 2]

# Resample to uniform 0.5 ns sampling
dt_ns = 5
t_orig = t_data.copy()
t_data = np.arange(5, t_orig.max(), dt_ns)
y_data_3m = np.interp(t_data, t_orig, y_data_3m)
y_data_3m_with_biastee = np.interp(t_data, t_orig, y_data_3m_with_biastee)

plt.semilogx(t_data, y_data_3m, label="3m no bias tee")
plt.semilogx(t_data, y_data_3m_with_biastee, label="3m with bias tee")

# plt.plot(square_pulse_3m_vs_12m[:, 0], square_pulse_3m_vs_12m[:, 1], label="3m")
# plt.plot(square_pulse_3m_vs_12m[:, 0], square_pulse_3m_vs_12m[:, 2], label="12m")

plt.xlabel("Time [ns]")
plt.ylabel("Voltage [V]")
plt.title("Step response of a 3m cable with and without bias tee")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
# fit data
fitting_base_fractions = [0.05, 0.001]
# fitting_base_fractions = [0.5, 0.05]
success, best_fractions, components, a_dc, best_rms = optimize_start_fractions(t_data, y_data_3m_with_biastee, fitting_base_fractions)

# plot data and fit
fig, axs = plot_fit(t_data, y_data_3m_with_biastee, components, a_dc)
